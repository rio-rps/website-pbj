...
<div class="main-menu menu-fixed menu-dark menu-accordion menu-shadow" data-scroll-to-active="true">
    <div class="main-menu-content">
        <ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation">
            ...
            <li class="nav-item {{ ($level1->id_laman != 0 && request()->url() == url('/lamandetail/'.$level1->JLaman->slug_laman)) ? 'active' : ''}}">

                <a href="{{ ($level1->id_laman == 0) ? '#' :  route('lamandetail.show',$level1->JLaman->slug_laman) }}">
                ...
                <ul class="menu-content">
                    <li class="nav-item ">
                        @if ($linkLevel2 !='0')
                    <li class="{{ (Str::contains(request()->url(), '/lamandetail/'.$level2->JLaman->slug_laman) && Str::contains(request()->url(), '/lamandetail/'.$level2->JLaman->slug_laman) && (Str::endsWith(request()->url(), '/edit') || Str::endsWith(request()->url(), '/'.$level2->JLaman->slug_laman))) ? 'active' : '' }}">
                        <a href="{{ ($linkLevel2 == 0) ? '#' :  route('lamandetail.show',$level2->JLaman->slug_laman) }}">
                            <span class="menu-title fonts11" title="{{ $level2->nm_menu }}" data-i18n="{{ $level2->nm_menu }}">{{ $level2->nm_menu }} </span>
                        </a>
                    </li>
                    ...
                    <li class="has-sub is-shown">
                        <a class="menu-item fonts11" href="#" title="{{ $level2->nm_menu }}" data-i18n="{{ $level2->nm_menu }}">{{ $level2->nm_menu }}</a>
                        ...
                        <ul class="menu-content">
                            <li class="{{ (Str::contains(request()->url(), '/lamandetail/'.$level3->JLaman->slug_laman) && (Str::endsWith(request()->url(), '/edit') || Str::endsWith(request()->url(), '/'.$level3->JLaman->slug_laman))) ? 'active' : '' }}">
                                <a class="menu-item fonts11" href="{{ ($linkLevel3 == 0) ? '#' :  route('lamandetail.show',$level3->JLaman->slug_laman) }}" title="{{ $level3->nm_menu }}" data-i18n="{{ $level3->nm_menu }}">{{$level3->nm_menu}}</a>
                            </li>
                        </ul>
                        @endif
