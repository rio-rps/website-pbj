                        ...
                        <div class="media-body">
                            ...
                            <h5 class="package-title"><a href="#">{{ $result->nm_dokumen }}</a>
                                <a class=" btn btn-primary btn-sm pull-right" target="_blank"
                                    href="{{ asset('upload-data/dokumen_laman/' . $result->file_dokumen) }}"><i
                                        class="fa fa-download"></i> Download</a>
                            </h5>
                            <div class="text-muted text-small">{{ $result->ket_dokumen }}</div>
                            <span class="badge badge-info" style="font-size: 10px;">Upload Date :
        ...
        @foreach ($row as $ddresult)
            <div class="d-flex align-items-center bs-callout-info callout-border-left callout-transparent mt-1 p-1">
                <span style="font-weight:bold;">{{ $ddresult->nm_gambar }}
                    <a href="{{ asset('upload-data/gambar_laman/' . $ddresult->file_gambar) }}" target="_blank"
                        class="badge badge-primary"><i class="fa fa-download"></i> Download</a>
                </span>
            </div>
            <img src="{{ asset('upload-data/gambar_laman/' . $ddresult->file_gambar) }}" class="img-thumbnail image"
